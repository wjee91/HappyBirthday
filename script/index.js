...
var Balloon = function (id) {
	...
	this.balloon = {
		...
		x: Math.random() * this.bundle.clientWidth,
		y: Math.random() * 100,
		amplitude: {
...
Balloon.prototype.setTimer = function () {
	this.move();
	var self = this;
	setTimeout(function () {self.setTimer()}, 20);
};
...
Balloon.prototype.move = function () {
	...
	if (balloon.y  + 125 > this.bundle.clientHeight) {
		balloon.y = 0;
		balloon.obj.style.top = 0;
	}
...
function createBalloons() {
    ...
    for (var i = 0; i < ids.length; i++) {
        createObj("img", "bundle", ids[i], srcs[i % 2]);
        new Balloon(ids[i]);
    }
}
